...
public class StringUtils {
	 ...
	 * deleted.
	 * 
	 * @deprecated liushen@May 31, 2010: 重复的方法，将删除; 应该使用
	...
	public static Integer avoidNull(Integer number) {
...
//	}
		...
		for (String property : properties) {
			...
			if (keyIndex != -1) {
				...
				String value = property.substring(
						keyIndex + fieldDelimiter.length()).trim();
	...
	public static Map<String, String> parseSimpleJsonStrAsMap(String src,
		...
		for (String property : properties) {
			int keyIndex = property.indexOf(fieldDelimiter);
			if (keyIndex != -1) {
				String key = StringUtils.trimQuote(property.substring(0,
						keyIndex).trim());
				String value = StringUtils.trimQuote(property.substring(
						keyIndex + fieldDelimiter.length()).trim());
				parsedMap.put(key, value);
			}
		}
	...
	public static String parseMapAsSimpleJsonStr(Map<String, String> srcMap,
			String propertyDelimiter, String fieldDelimiter, boolean bHasKH) {
		...
		for (int i = 0; i < keys.length; i++) {
			sb.append(StringUtils.quotes(keys[i]))
					.append(fieldDelimiter)
					.append(StringUtils.quotes(StringUtils.avoidNull(srcMap
							.get(keys[i])))).append(propertyDelimiter);
		}
		if (bHasKH) {
			sb.append("}");
		}
...
//		String NODE_REGEX = "<(?<nodeName>[^>]+?)>(?<nodeValue>[\\w\\W]+?)</\\k<nodeName>>";
//		Pattern pt = Pattern.compile(NODE_REGEX, Pattern.CASE_INSENSITIVE);
...
//	}
	...
	public static String parseMapAsSimpleXMLStr(Map<String, String> srcMap) {
		...
		for (int i = 0; i < keys.length; i++) {
			sb.append("<").append(keys[i]).append(">")
					.append(StringUtils.avoidNull(srcMap.get(keys[i])))
	 ...
	 * @since fangxiang @ Oct 28, 2010
	 */
	// TODO: liushen@Mar 2, 2011: 该方法需要增加对https的支持。
	 ...
	 */
	// TODO: liushen@Mar 2, 2011: 该方法需要增加对rtmps、rtmpt等的支持。
...
//	}
	...
	public static String join(Map<String, ?> entities, String separator) {
		...
		for (int i = 0; i < keys.length; i++) {
			buf.append(keys[i])
					.append("=")
					.append(StringUtils.avoidNull(entities.get(keys[i])
	...
	public static String getDomain(String url) {
		...
		int i = url.indexOf("/");
		String host = i > 0 ? url.substring(0, i) : url;
		return host;
	}
	...
	public static boolean isLetter(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			boolean isLetter = ((str.charAt(i) <= 'Z' && str.charAt(i) >= 'A') || (str
					.charAt(i) <= 'z' && str.charAt(i) >= 'a'));
			if (!isLetter) {
				return false;
			}
		}
	 ...
	 * @return
	 */
	public static String completeDate(String time) {
		if (time.length() == 4) {
			time += "-01-01";
	...
	/**
	 * 百科日期  弹窗显示处理时间的值为"0"为"未知"为"至今"的情况
	 * @param time
	 * @return
	 */
	public static String remove0AndWZ(String time) {
		time = trim(time);
		if ("0".equals(time) || "未知".equals(time) || time == null || "至今".equals(time)) {
			return "";
		} 
		if (isEmpty(time)) {
			return "";
	...
	/**
	 * 与isEmpty相反,且不为0或未知或至今,主要用于百科时间的显示格式,有些时间在数据库中为"0",有些为"未知",有些为"至今"
	 * @param str
	 * @return
	 */
	public static boolean isNotEmptyAndZero(String str) {
		boolean flag = false == isEmpty(str);
		if ("0".equals(str)||"未知".equals(str)||str == null||"至今".equals(str)) {
			flag = false;
			return flag;
		}
		return flag;
	...
	public static String engoreChar(String str){
		if(str == null)
			return "";
		str = str.replaceAll("<", "");
		str = str.replaceAll(">", "");
		...
		str = str.replaceAll("；", "");
		str = str.replaceAll("@", "");
		str = str.replaceAll("\\^", ""); //^符号需要转义
		return str.trim();
	}	
	
	public static void main(String[] args) {
//		System.out.println(engoreChar("阿里山肯德基发塑料袋副^^"));
	}
